...
async function uploadImages(page, absPaths) {
    ...
    if (page.setFileInput) {
      ...
      `);
            ...
            // 本机补丁 2026-09-13：扩展 v1.0.24 的 set-file-input 走 fileChooser，隐藏 input 必超时 → 同样回落 DataTransfer
            if (msg.includes('Unknown action') || msg.includes('not supported') || msg.includes('Not allowed') || msg.includes('fileChooserOpened')) {
    ...
    (async () => {
      ...
      try { input.files = dt.files; } catch (e) { Object.defineProperty(input, 'files', { value: dt.files, writable: false }); }
