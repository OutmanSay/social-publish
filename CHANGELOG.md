...
## 2026-09-18.6 📦
- **公众号标题层级**：文章标题在草稿「标题」栏，正文标题最高只到 h2。正文用了 `#` 的文章整体降一级（`#`→h2 绿线、`##`→h3 绿条、`###`→h4 加粗），不再出现一级和二级长得一样；没用 `#` 的文章显示不变。新增 `####` 支持；代码块里的 `#` 不算标题。
...
- **新增新粗野主义（Neo-Brutalism）主题并统一全系 1px 细线**：
  ...
  - 全系主题小标题底线统一收窄为 `1px` 极细实线（告别 `2px` 马克笔手绘感），呈现正规出版物般的精细字感与留白。
...
- **新增全平台一键并发主控调度器**：`publish_all.py`。
  ...
  - 微博走纯 HTTP 毫秒级先发（不占浏览器），剩余 6 个平台由 2 个并发线程池流水线动态消费，总耗时从 3 分钟稳定压至 30~45 秒。
  - 统一资产单次预处理：封面图单次压制全员共享复用、标题与摘要自动从 frontmatter 提取、多平台正文自适应分流。
